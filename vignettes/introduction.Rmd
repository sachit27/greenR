...
This function takes the OSM data, an output Coordinate Reference System (CRS) code, and a distance-decay parameter `D` in metres. It finds the nearest green area and tree within `buffer_distance` metres of each street segment, applies exponential distance decay, and scales the mean score from 0 to 1 across the supplied streets. Missing green areas or trees contribute zero. When the output CRS is geographic, such as EPSG:4326, distances are calculated in a local metric CRS before the result is transformed back.

```{r, eval = FALSE}

green_index <- calculate_green_index(data, crs_code = 4326, D = 100)
...
greenR enables rapid analysis and visualization of global 1m Canopy Height Model (CHM) data from Meta & WRI's ALS GEDI v6 dataset. Analyze, mosaic, crop, and visualize canopy heights for any city or AOI - including tree cover fraction and AOI size (Mollweide km^2) with maps and histograms.
...
The `analyze_and_visualize_uhi()` function performs comprehensive Urban Heat Island analysis by integrating satellite-derived Land Surface Temperature (LST) data with environmental features from OpenStreetMap. This function automates the entire workflow from data acquisition to visualization.

**Key Features:**

- Automatic retrieval of thermal imagery (ECOSTRESS at 70m or Landsat 8/9 at 100m) via Microsoft Planetary Computer
- Integration with OSM data (green spaces, trees, buildings, water bodies)
- H3 hexagonal grid aggregation for consistent spatial analysis
- Getis-Ord Gi* hotspot analysis with statistical significance testing
- Moran's I spatial autocorrelation assessment
- Interactive Leaflet maps and publication-ready ggplot2 visualizations
- Export functions for GeoJSON, GeoPackage, CSV, and Shapefile formats

**Note:** This function requires internet access to download satellite imagery and OSM data.

```{r, eval = FALSE}
# Basic UHI analysis for a city
result <- analyze_and_visualize_uhi(
  location = "Basel, Switzerland",
  date_range = c("2023-06-01", "2023-08-31"),
  hex_resolution = 10,
  thermal_source = "auto"  # Tries ECOSTRESS first, then Landsat
)

# View the interactive map (opens in browser/viewer)
result$maps$interactive

# View static publication-ready maps
print(result$maps$combined)  # Combined 4-panel map
print(result$maps$scatter)   # Scatter plot dashboard
...
- `results`: An sf object with hexagon-level metrics (LST, green coverage, built-up, hotspot categories)
- `maps`: Interactive Leaflet map and static ggplot2 maps
- `stats`: Descriptive statistics, correlations, regression results, and Moran's I
- `meta`: Processing metadata and data source information
- `export_geojson()` and `export_results()`: Functions for data export

```{r, eval = FALSE}
# Export results to multiple formats
result$export_results("basel_uhi", formats = c("geojson", "csv", "gpkg"))

# Using a bounding box instead of city name
result_bbox <- analyze_and_visualize_uhi(
  location = c(7.55, 47.53, 7.65, 47.58),  # xmin, ymin, xmax, ymax

  date_range = c("2023-07-01", "2023-07-31"),
  hex_resolution = 9  # Larger hexagons for faster processing
)

# With optional GHSL built-up data for more accurate built coverage
result_ghsl <- analyze_and_visualize_uhi(
  location = "Zurich, Switzerland",
  date_range = c("2023-06-01", "2023-08-31"),
  ghsl_path = "path/to/ghsl_built.tif",
  composite_scenes = TRUE,  # Composite multiple scenes to reduce cloud gaps
  max_scenes = 5
)
```
...
The `uh_svf()` function computes point-based and street-segment-level sky-view factors (SVF). SVF represents the horizontal projection of visible sky accounting for the Lambert cosine law of diffuse solar irradiance (Johnson & Watson 1984, Oke 1987). It calculates sky openness and obstruction from both terrain and buildings. 

It supports a hybrid workflow, automatically fetching building shapes from the Global Building Atlas S3 dataset, digital elevation models from AWS terrain tiles, and local canopy height models, or relying entirely on local datasets.

**Key Features:**
- Parallelized horizon-angle ray-casting across any number of azimuth directions.
- Generates interactive Leaflet maps with dynamic layers and linked legends.
- Generates fully interactive 3D WebGL explorers using Deck.gl extruded prisms.
- Generates publication-ready static cartographic maps and distribution curves.
- Computes radial horizon-line profiles to render Simulated Sky Obstruction Profiles for individual observer locations.

``` {r, eval = FALSE}
library(greenR)

# Basic SVF screening using AWS terrain and Global Building Atlas (GBA) building footprints
result_svf <- uh_svf(
  city_name = "Basel",
...
# View the calculated microclimate statistics and summaries
print(result_svf$analytics)
```

# 20. Multi-Criteria Urban Heat and Canopy Decision Support Suite

The `uh_decision()` function is a high-level orchestration engine that automates the entire greenR Canyon Suite workflow. It compiles and merges multi-source spatial data to identify urban heat vulnerability and locate the highest-priority areas for greening interventions (such as tree-planting corridors).

It performs Multi-Criteria Decision Analysis (MCDA) based on Land Surface Temperature (LST), Population density, Sentinel-2 NDVI, Canopy Height Model (CHM) canopy cover, building footprints, and tree densities.

**Key Features:**
- Emulates street canyon orientations and solar bearings.
- Computes population-weighted spatial priority Gini inequality metrics with bootstrap confidence intervals.
- Segregates spatial priority zones into four actionable policy planning quadrants (e.g. "Hotter + Plantable").
- Generates 12 different publication-ready bivariate maps, 3D WebGL explorers, and multi-layer interactive Leaflet dashboards.

``` {r, eval = FALSE}
library(greenR)

# Run the complete decision support MCDA suite for a city
result_decision <- uh_decision(
...
# Explore the resulting data layers
# Contains priority_grid (hex level), block_priority (superblock level), and canyon_priority (street corridor level)
print(names(result_decision))
```
